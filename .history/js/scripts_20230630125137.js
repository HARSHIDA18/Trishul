...
fetch("data.csv")
  ...
  .then((csvData) => {
    ...
    var lines = csvData.split("\n");
    var headers = lines[0].split(",");
    var load1Index = headers.indexOf("Pressure1");
    var load2Index = headers.indexOf("Pressure2");
    var data = [];

    for (var i = 1; i < lines.length; i++) {
      var currentLine = lines[i].split(",");
      var load1 = parseFloat(currentLine[load1Index]);
      var load2 = parseFloat(currentLine[load2Index]);
      data.push([load1, load2]);
    }
    ...
    areaChart.updateOptions({
      ...
      labels: data.map((point1) => point1[0]),
    });
  });
